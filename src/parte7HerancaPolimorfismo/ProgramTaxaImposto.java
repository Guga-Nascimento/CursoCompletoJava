package parte7HerancaPolimorfismo;

import java.util.ArrayList;
import java.util.List;
import java.util.Locale;
import java.util.Scanner;

import parte7HerancaPolimorfismo.entidades.CadastroContribuinte;
import parte7HerancaPolimorfismo.entidades.PessoaFisica;
import parte7HerancaPolimorfismo.entidades.PessoaJuridica;

public class ProgramTaxaImposto {

	public static void main(String[] args) {
		Locale.setDefault(Locale.US);
		Scanner sc = new Scanner(System.in);
		/*
		  Fazer um programa para ler os dados de N contribuintes (N fornecido pelo
		  usuário), os quais podem ser pessoa física ou pessoa jurídica, e depois
		  mostrar o valor do imposto pago por cada um, bem como o total de imposto
		  arrecadado. Os dados de pessoa física são: nome, renda anual e gastos com
		  saúde. Os dados de pessoa jurídica são nome, renda anual e número de
		  funcionários. As regras para cálculo de imposto são as seguintes: Pessoa
		  física: pessoas cuja renda foi abaixo de 20000.00 pagam 15% de imposto.
		  Pessoas com renda de 20000.00 em diante pagam 25% de imposto. Se a pessoa
		  teve gastos com saúde, 50% destes gastos são abatidos no imposto. Exemplo:
		  uma pessoa cuja renda foi 50000.00 e teve 2000.00 em gastos com saúde, o
		  imposto fica: (50000 * 25%) - (2000 * 50%) = 11500.00 Pessoa jurídica:
		  pessoas jurídicas pagam 16% de imposto. Porém, se a empresa possuir mais de
		  10 funcionários, ela paga 14% de imposto. Exemplo: uma empresa cuja renda foi
		  400000.00 e possui 25 funcionários, o imposto fica: 400000 * 14% = 56000.00
		 */
		System.out.print("Digite o número de contribuintes : ");
		int n = sc.nextInt();
		
		List<CadastroContribuinte> contribuintes = new ArrayList<>();

		for (int i = 1; i <= n; i++) {
			
			System.out.print("Informe o nome do contribuinte: ");
			sc.nextLine();
			String nome = sc .nextLine();
			System.out.print("Informe o rendimento anual: ");
			Double rendimentoAnual = sc.nextDouble();
			System.out.print("Pessoa física ou jurídica?  (f ou j): ");
			char ch = sc.next().charAt(0);
			if (ch =='f' ) {
				System.out.print("Informe valor gasto com saúde : ");
				Double gastoComSaude = sc.nextDouble();
				contribuintes.add(new PessoaFisica(nome, rendimentoAnual, gastoComSaude));
				System.out.println("//////////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\");
				
			}else if(ch == 'j') {
				System.out.print("Informe o número de funcionários: ");
				Integer numeroFuncionarios = sc.nextInt();
				contribuintes.add(new PessoaJuridica(nome, rendimentoAnual, numeroFuncionarios));
				System.out.println("//////////////////////////////////////////////\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\");
				
			}else {
				System.err.println("Dados inválidos \n\nREINICIE O PROGRAMA. ");
			}
		}
			double soma = 0;
			for (CadastroContribuinte contrinuinte : contribuintes) {
				System.out.println();
				System.out.println(contrinuinte.getNome() +String.format("  Valor : %.2f ",contrinuinte.taxaImposto()));
				soma += contrinuinte.taxaImposto();
			}
			System.out.println();
			System.out.println(String.format("Total arrecadado: %.2f\n", soma));

	

		sc.close();
	}

}
